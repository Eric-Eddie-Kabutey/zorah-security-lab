								...
								<div
									...
									{columns.map((column) => (
										<div key={column.title} className='p-8'>
											...
											<p className='mt-1 mb-6 text-sm text-foreground/70'>
												{column.description}
											</p>

											<div
												className={cn(
													column.linkLayout === 'grid'
												...
												{column.links.map((link) => (
													<Link
														key={link.title}
														href={link.href}
														className='block group hover:bg-gray-200 p-2 rounded-lg'>
														<div className='flex items-start gap-x-3'>
															{/* You can add icon logic back here if needed */}
															{link.hasBnt ? "" : <Image
																src={link.imgUrl ?? '/asset/icons/setting.png'}
																alt={link.title}
																width={40}
																height={20}
															...
															<div>
																{link.hasBnt ? (
																	<button className='bg-gray-600 py-4 px-2  text-white text-sm rounded-lg'>
																		{link.title}
																	</button>
																) : (
																	<p className='font-normal text-foreground group-hover:text-foreground/70 transition-colors'>
																		{link.title}
																	</p>
																)}
															</div>
